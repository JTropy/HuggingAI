...
function isBaseQOAuthProvider(provider) {
  return [provider?.slug, provider?.name, provider?.icon].some((value) => {
    const normalized = String(value || '')
      .toLowerCase()
      .replace(/[^a-z0-9]/g, '');
    return ['baseq', 'qq', 'tencentqq'].some((keyword) =>
...
export function getCustomOAuthProviderIcon(provider, size = 20) {
  if (isBaseQOAuthProvider(provider)) {
    return getOAuthProviderIcon('qq', size);
  }

  return getOAuthProviderIcon(provider?.icon || '', size);
}
